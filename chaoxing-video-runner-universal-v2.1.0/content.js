(() => {
  ...
  async function processPlayback(token) {
    while (await ensureRunConditions(token)) {
      ...
      const liveEntry = (type === "document" ? entries.documents : entries.videos)
        .find((entry) => entry.jobId === task.jobId);
      if (!liveEntry) {
        if (type === "document") {
          const retries = Number(state.retries?.[task.jobId] || 0);
          if (retries >= CONFIG.maxRetries) {
            await pauseForDocumentFailure(task, lesson, "章节中未找到已扫描的文档节点", retries);
            return false;
          }
          const nextRetries = retries + 1;
          await saveState({ retries: { ...(state.retries || {}), [task.jobId]: nextRetries } });
          await addLog(`${task.name}：未找到文档节点，10秒后重试 ${nextRetries}/${CONFIG.maxRetries}`, "warn");
          await sleep(CONFIG.retryDelayMs);
        } else {

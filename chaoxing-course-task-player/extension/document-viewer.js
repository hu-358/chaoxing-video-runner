(() => {
  ...
  function isViewerInactive() {
    return document.hidden || pageFrozen;
  }

  function createActiveWatchdog(delayMs, callback) {
    ...
    const schedule = () => {
      if (cancelled || timer || isViewerInactive()) return;
      startedAt = Date.now();
    ...
    function sync() {
      isViewerInactive() ? pause() : schedule();
    }
    function freeze() {
      pageFrozen = true;
      sync();
    ...
    schedule();
    return { cancel, pause, resume: schedule };
  }
  ...
  function getParentOrigin() {
    ...
    for (const candidate of candidates) {
      try {
        ...
        if (isTrustedChaoxingOrigin(origin)) return origin;
      } catch {
        // Try the next browser-provided parent reference.
      }
    }
    return null;
  }
  ...
  function post(type, payload = {}) {
    ...
    if (targetOrigin) {
      window.top.postMessage({ source: VIEWER_SOURCE, type, ...payload }, targetOrigin);
    }
  }
  ...
  function waitUntilReady(runId) {
    ...
    return new Promise((resolve) => {
      ...
      watchdog = createActiveWatchdog(READY_TIMEOUT_MS, () => finish(viewerReady()));
      check();
    });
  }
  ...
  function advancePanPages(panView, runId, jobId) {
    return new Promise((resolve, reject) => {
      ...
      document.addEventListener("resume", wake);
      window.addEventListener("pageshow", wake);
      watchdog = createActiveWatchdog(LAYOUT_TIMEOUT_MS, () => {
  ...
  function waitForPageChange(before, runId) {
    ...
    return new Promise((resolve) => {
      ...
      document.addEventListener("resume", wake);
      window.addEventListener("pageshow", wake);
      watchdog = createActiveWatchdog(15_000, () => {
        if (settled) return;
        settled = true;
        observer.disconnect();
        document.removeEventListener("visibilitychange", wake);
  ...
  window.addEventListener("message", (event) => {
    ...
    if ((event.source !== window.top && event.source !== window.parent)
      || !isTrustedChaoxingOrigin(event.origin)

(() => {
  ...
  async function pauseForVerificationConflict(lesson, conflict) {
    ...
    await showDesktopNotification(
      "学习通最终核验已暂停",
      `${state.name}：${lesson.title} 连续${CONFIG.maxRetries}次存在任务点数量冲突，请检查页面。`,
      2
    );
    await releaseLock();
  }
    ...
    if (state.running && hasLegacyExtensionConflict()) {
      ...
      if (!lock?.acquired) {
        ...
        await saveState({
          ...
          phase: "scanning",
          resumePhase: "scanning",
          status: document.wasDiscarded
            ? "页面曾被浏览器丢弃，正在恢复并重新扫描课程"
            : "页面重新载入，正在重新扫描课程",
          catalog: [],
          catalogComplete: false,
          scanIndex: 0,
        ...
        await addLog(
          document.wasDiscarded
            ? "检测到页面曾被浏览器丢弃，已恢复状态并重新扫描全部课程章节。"
            : "页面或Edge已重新载入，按设置重新扫描全部课程章节。",
          "success"
        );
        runToken += 1;
        void launchLoop(runToken);

(() => {
  ...
  async function verifyCatalog(token) {
    ...
    const verificationLessons = (state.catalog || []).map((lesson) => ({
      ...lesson,
      videos: [...(lesson.videos || [])],
      documents: [...(lesson.documents || [])]
    }));
    const totalTasks = verificationLessons.reduce(
    ...
    await addLog(`开始最终核验：${verificationLessons.length}个章节，共${totalTasks}个视频与文档任务。`, "success");
    for (let index = 0; index < verificationLessons.length; index += 1) {
      ...
      await addLog(
        `最终核验章节 ${index + 1}/${verificationLessons.length}：${lesson.section} ${lesson.title}（${lesson.videos.length}个视频，${lesson.documents.length}个文档）`
      );
      const opened = getCurrentLessonId() === lesson.lessonId
        ? true
        : await navigateToLesson(lesson, token);
      ...
      const entries = await waitForStableMediaEntries(token, true);
      const liveVideos = new Map(entries.videos.map((entry) => [entry.jobId, entry]));
      const liveDocuments = new Map(entries.documents.map((entry) => [entry.jobId, entry]));
      for (const task of lesson.videos) {
        checkedTasks += 1;
        const live = liveVideos.get(task.jobId);
        const result = live ? (isVideoComplete(live) ? "已完成" : "未完成") : "节点缺失";
      ...
      for (const task of lesson.documents) {
        checkedTasks += 1;
        const live = liveDocuments.get(task.jobId);
        const result = live ? (isDocumentComplete(live) ? "已完成" : "未完成") : "节点已失效，清单将自动同步";
